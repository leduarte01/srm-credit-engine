export type Lang = 'en' | 'pt';

export const TRANSLATIONS = {
  en: {
    // Navigation
    nav_receivables: 'Receivables',
    nav_config: 'Config',
    nav_assignors: 'Assignors',
    nav_exchange_rates: 'Exchange Rates',
    nav_soon: 'soon',
    // Dashboard
    page_title: 'Operator panel',
    page_subtitle: 'Review receivables, simulate pricing and settle outstanding balances.',
    btn_refresh: 'Refresh',
    btn_refreshing: 'Refreshing…',
    // KPI
    kpi_total: 'Total',
    kpi_pending: 'Pending',
    kpi_settled: 'Settled',
    kpi_cancelled: 'Cancelled',
    // Pricing simulator
    sim_title: 'Pricing simulator',
    sim_subtitle: 'Computes present value, settlement value and applied rate without persisting.',
    sim_product_code: 'Product code',
    sim_currency: 'Currency',
    sim_face_value: 'Face value',
    sim_issue_date: 'Issue date',
    sim_due_date: 'Due date',
    sim_btn_simulate: 'Simulate',
    sim_btn_simulating: 'Simulating…',
    sim_present_value: 'Present value',
    sim_settlement_value: 'Settlement value',
    sim_eff_rate: 'Effective rate / month',
    sim_base_rate: 'Base rate / month',
    sim_spread: 'Spread / month',
    sim_term: 'Term (months)',
    sim_fx: 'FX applied',
    // Filters
    filter_assignor: 'Assignor (document)',
    filter_assignor_placeholder: 'Only digits',
    filter_product: 'Product',
    filter_currency: 'Currency',
    filter_currency_any: 'Any',
    filter_status: 'Status',
    filter_status_any: 'Any',
    filter_reset: 'Reset',
    // Table
    col_reference: 'Reference',
    col_assignor: 'Assignor',
    col_product: 'Product',
    col_face_value: 'Face value',
    col_issue: 'Issue',
    col_due: 'Due',
    col_status: 'Status',
    btn_settle: 'Settle',
    btn_settling: 'Settling…',
    btn_cancel: 'Cancel',
    btn_new_receivable: 'New Receivable',
    // New receivable modal
    modal_new_title: 'New Receivable',
    modal_field_assignor_doc: 'Assignor document (CPF/CNPJ)',
    modal_field_product: 'Product',
    modal_field_amount: 'Face value',
    modal_field_currency: 'Currency',
    modal_field_issue: 'Issue date',
    modal_field_due: 'Due date',
    modal_field_reference: 'External reference',
    modal_btn_submit: 'Create',
    modal_btn_cancel: 'Cancel',
    modal_creating: 'Creating…',
    modal_created: 'Receivable created.',
    no_receivables: 'No receivables match the current filters.',
    loading_receivables: 'Loading receivables…',
    showing_x_of_y: 'Showing {shown} of {total} receivables.',
    showing_range: 'Showing {from}–{to} of {total} receivables.',
    pagination_prev: 'Previous',
    pagination_next: 'Next',
    // Status badge
    status_pending: 'Pending',
    status_settled: 'Settled',
    status_cancelled: 'Cancelled',
    // Help modal
    help_btn: 'Help',
    help_title: 'How to use Credit Engine',
    help_kpi_title: 'KPI Cards',
    help_kpi_body:
      'Shows the real-time count of receivables by status (Pending, Settled, Cancelled). The bar inside each card shows the proportional share.',
    help_sim_title: 'Pricing Simulator',
    help_sim_body:
      'Calculates present value and settlement value for a receivable without saving to the database. Enter the product code, face value, currency and dates, then click Simulate.',
    help_filters_title: 'Filters',
    help_filters_body:
      'Filter receivables by assignor document (CPF/CNPJ digits only), product code, currency or status. Click Reset to clear all filters.',
    help_table_title: 'Receivables Table',
    help_table_body:
      'Lists all receivables matching the current filters. For PENDING receivables, Settle creates a settlement locking the value, and Cancel voids the receivable.',
    help_close: 'Close',
    // Assignors page
    assignors_title: 'Assignors',
    assignors_subtitle: 'Register and manage assignors (clients) who submit receivables.',
    btn_new_assignor: 'New Assignor',
    assignors_col_document: 'Document',
    assignors_col_legal_name: 'Legal Name',
    assignors_empty: 'No assignors registered yet.',
    assignors_loading: 'Loading assignors…',
    assignor_form_document: 'Document (CPF/CNPJ)',
    assignor_form_document_placeholder: 'Digits only',
    assignor_form_legal_name: 'Legal Name',
    assignor_form_legal_name_placeholder: 'Company legal name',
    assignor_form_submit: 'Create Assignor',
    assignor_form_submitting: 'Creating…',
    assignor_form_title: 'New Assignor',
    // Exchange rates page
    fx_title: 'Exchange Rates',
    fx_subtitle: 'Register FX rates used for USD-denominated receivable pricing.',
    btn_new_fx_rate: 'New Rate',
    fx_col_pair: 'Pair',
    fx_col_rate: 'Rate',
    fx_col_valid_from: 'Valid From',
    fx_col_valid_to: 'Valid To',
    fx_col_open: 'Open',
    fx_pair_label: 'Currency pair',
    fx_loading: 'Loading rates…',
    fx_empty: 'No rates registered for this pair.',
    fx_form_title: 'New FX Rate',
    fx_form_base: 'Base currency',
    fx_form_quote: 'Quote currency',
    fx_form_rate: 'Rate',
    fx_form_valid_from: 'Valid from',
    fx_form_valid_to: 'Valid to (optional)',
    fx_form_submit: 'Create Rate',
    fx_form_submitting: 'Creating…',
  },
  pt: {
    // Navegação
    nav_receivables: 'Recebíveis',
    nav_config: 'Configurações',
    nav_assignors: 'Cedentes',
    nav_exchange_rates: 'Taxas de Câmbio',
    nav_soon: 'em breve',
    // Dashboard
    page_title: 'Painel do operador',
    page_subtitle: 'Revise recebíveis, simule precificação e liquide saldos pendentes.',
    btn_refresh: 'Atualizar',
    btn_refreshing: 'Atualizando…',
    // KPI
    kpi_total: 'Total',
    kpi_pending: 'Pendente',
    kpi_settled: 'Liquidado',
    kpi_cancelled: 'Cancelado',
    // Simulador
    sim_title: 'Simulador de precificação',
    sim_subtitle: 'Calcula valor presente, valor de liquidação e taxa aplicada sem persistir.',
    sim_product_code: 'Código do produto',
    sim_currency: 'Moeda',
    sim_face_value: 'Valor de face',
    sim_issue_date: 'Data de emissão',
    sim_due_date: 'Data de vencimento',
    sim_btn_simulate: 'Simular',
    sim_btn_simulating: 'Simulando…',
    sim_present_value: 'Valor presente',
    sim_settlement_value: 'Valor de liquidação',
    sim_eff_rate: 'Taxa efetiva / mês',
    sim_base_rate: 'Taxa base / mês',
    sim_spread: 'Spread / mês',
    sim_term: 'Prazo (meses)',
    sim_fx: 'FX aplicado',
    // Filtros
    filter_assignor: 'Cedente (documento)',
    filter_assignor_placeholder: 'Somente dígitos',
    filter_product: 'Produto',
    filter_currency: 'Moeda',
    filter_currency_any: 'Qualquer',
    filter_status: 'Status',
    filter_status_any: 'Todos',
    filter_reset: 'Limpar',
    // Tabela
    col_reference: 'Referência',
    col_assignor: 'Cedente',
    col_product: 'Produto',
    col_face_value: 'Valor de face',
    col_issue: 'Emissão',
    col_due: 'Vencimento',
    col_status: 'Status',
    btn_settle: 'Liquidar',
    btn_settling: 'Liquidando…',
    btn_cancel: 'Cancelar',
    btn_new_receivable: 'Novo Recebível',
    // Modal novo recebível
    modal_new_title: 'Novo Recebível',
    modal_field_assignor_doc: 'Documento do cedente (CPF/CNPJ)',
    modal_field_product: 'Produto',
    modal_field_amount: 'Valor de face',
    modal_field_currency: 'Moeda',
    modal_field_issue: 'Data de emissão',
    modal_field_due: 'Data de vencimento',
    modal_field_reference: 'Referência externa',
    modal_btn_submit: 'Criar',
    modal_btn_cancel: 'Cancelar',
    modal_creating: 'Criando…',
    modal_created: 'Recebível criado.',
    no_receivables: 'Nenhum recebível encontra os filtros atuais.',
    loading_receivables: 'Carregando recebíveis…',
    showing_x_of_y: 'Exibindo {shown} de {total} recebíveis.',
    showing_range: 'Exibindo {from}–{to} de {total} recebíveis.',
    pagination_prev: 'Anterior',
    pagination_next: 'Próxima',
    // Badge de status
    status_pending: 'Pendente',
    status_settled: 'Liquidado',
    status_cancelled: 'Cancelado',
    // Modal de ajuda
    help_btn: 'Ajuda',
    help_title: 'Como usar o Credit Engine',
    help_kpi_title: 'Cards de KPI',
    help_kpi_body:
      'Exibe a contagem em tempo real de recebíveis por status (Pendente, Liquidado, Cancelado). A barra dentro de cada card mostra a proporção em relação ao total.',
    help_sim_title: 'Simulador de Precificação',
    help_sim_body:
      'Calcula o valor presente e o valor de liquidação de um recebível sem salvar no banco de dados. Informe o código do produto, valor de face, moeda e datas, depois clique em Simular.',
    help_filters_title: 'Filtros',
    help_filters_body:
      'Filtre recebíveis por documento do cedente (somente dígitos do CPF/CNPJ), código do produto, moeda ou status. Clique em Limpar para resetar todos os filtros.',
    help_table_title: 'Tabela de Recebíveis',
    help_table_body:
      'Lista todos os recebíveis que correspondem aos filtros atuais. Para recebíveis PENDENTES, Liquidar cria uma liquidação travando o valor, e Cancelar anula o recebível.',
    help_close: 'Fechar',
    // Página Cedentes
    assignors_title: 'Cedentes',
    assignors_subtitle: 'Cadastre e gerencie cedentes (clientes) que submetem recebíveis.',
    btn_new_assignor: 'Novo Cedente',
    assignors_col_document: 'Documento',
    assignors_col_legal_name: 'Razão Social',
    assignors_empty: 'Nenhum cedente cadastrado ainda.',
    assignors_loading: 'Carregando cedentes…',
    assignor_form_document: 'Documento (CPF/CNPJ)',
    assignor_form_document_placeholder: 'Somente dígitos',
    assignor_form_legal_name: 'Razão Social',
    assignor_form_legal_name_placeholder: 'Nome legal da empresa',
    assignor_form_submit: 'Criar Cedente',
    assignor_form_submitting: 'Criando…',
    assignor_form_title: 'Novo Cedente',
    // Página Taxas de Câmbio
    fx_title: 'Taxas de Câmbio',
    fx_subtitle: 'Cadastre taxas FX usadas para precificação de recebíveis em USD.',
    btn_new_fx_rate: 'Nova Taxa',
    fx_col_pair: 'Par',
    fx_col_rate: 'Taxa',
    fx_col_valid_from: 'Válido De',
    fx_col_valid_to: 'Válido Até',
    fx_col_open: 'Aberta',
    fx_pair_label: 'Par de moedas',
    fx_loading: 'Carregando taxas…',
    fx_empty: 'Nenhuma taxa cadastrada para este par.',
    fx_form_title: 'Nova Taxa FX',
    fx_form_base: 'Moeda base',
    fx_form_quote: 'Moeda cotação',
    fx_form_rate: 'Taxa',
    fx_form_valid_from: 'Válido de',
    fx_form_valid_to: 'Válido até (opcional)',
    fx_form_submit: 'Criar Taxa',
    fx_form_submitting: 'Criando…',
  },
} as const;

export type TranslationKey = keyof typeof TRANSLATIONS.en;
